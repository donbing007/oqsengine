...
public class QueryLimitCommitidByConditionsExecutor extends AbstractMasterExecutor<Conditions, Collection<EntityRef>> {
    ...
    private String buildSQL(String where) {
        ...
        // select id,pref,cref, JSON_UNQUOTE(JSON_EXTRACT(attribute, '$.sort')) as sort from oqsbigentity where (entity = ? and commitid = ?) and (where)
        sql.append("SELECT ")
            .append(String.join(",", FieldDefine.ID, FieldDefine.PREF, FieldDefine.CREF));
        if (!sort.isOutOfOrder()) {
            /**
