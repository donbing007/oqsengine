...
public class ManagementWithCalculatorTest {
    ...
    private enum Compage {
        ...
        public static boolean compareTwoValue(Object a, Object b, Compage compare) {
    ...
    private final Map<Long, AbstractMap.SimpleEntry<Object, Compage>> expectedResult = new HashMap<>();
        ...
        if (operationResult.getResultStatus().equals(ResultStatus.SUCCESS)
            ...
            expectedResult.forEach((key, value) -> {
                ...
                Assertions.assertTrue(
                    Compage.compareTwoValue(value.getKey(), valueOp.get().getValue(), value.getValue()));
    ...
    private void setExpectedResult(Long expectedValue) {
        if (expectedValue != null) {
            expectedResult.put(Long.MAX_VALUE - 1, new AbstractMap.SimpleEntry<>(expectedValue * 3, Compage.EQ));
            expectedResult.put(Long.MAX_VALUE - 2, new AbstractMap.SimpleEntry<>(expectedValue * 3 / 2, Compage.EQ));
            expectedResult
                .put(Long.MAX_VALUE - 5, new AbstractMap.SimpleEntry<>(expectedValue + "-", Compage.START_WITH));
        } else {
            expectedResult.put(Long.MAX_VALUE - 1, new AbstractMap.SimpleEntry<>(0, Compage.EQ));
            expectedResult.put(Long.MAX_VALUE - 2, new AbstractMap.SimpleEntry<>(1, Compage.EQ));
            expectedResult.put(Long.MAX_VALUE - 5, new AbstractMap.SimpleEntry<>("0", Compage.EQ));
        }

        expectedResult.put(Long.MAX_VALUE - 3,
            new AbstractMap.SimpleEntry<>(TimeUtils.convert(System.currentTimeMillis()), Compage.NOTHING));
        expectedResult.put(Long.MAX_VALUE - 4, new AbstractMap.SimpleEntry<>(0L, Compage.NOTHING));

        expectedResult.put(Long.MAX_VALUE - 6, new AbstractMap.SimpleEntry<>(0L, Compage.NOTHING));
        expectedResult.put(Long.MAX_VALUE - 7, new AbstractMap.SimpleEntry<>(0L, Compage.NOTHING));
    }
