...
public class SQLConstant {
    ...
    public static final String BATCH_SQL = "%s into %s (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) values";
    public static final String DELETE_SQL = "delete from %s where id = ?";
    /**
     * %s 顺序为 where 条件, 排序.
     * NO_ENTITY表示不会增加entity的过滤.
     */
    public static final String SELECT_SQL = "select id, pref, cref, oqsmajor %s from %s where %s %s limit ?,? option max_matches=?,max_query_time=?,ranker=none,low_priority";
    ...
    public static final String SELECT_FROM_ID_SQL = "select id, pref, cref, entity, tx, commitid, jsonfields, maintainid, time from %s where id = ? option max_matches=1,ranker=none,low_priority";
