...
//        final ManticoreStorageEntity oldManticoreStorageEntity;
//        oldManticoreStorageEntity = (ManticoreStorageEntity) writeTransactionExecutor.execute(new ResourceTask() {
//            @Override
//            public Object run(TransactionResource resource, ExecutorHint hint) throws SQLException {
//
//                Optional<ManticoreStorageEntity> oldStorageEntityOptional =
...
//        });
//
//        if (oldManticoreStorageEntity != null) {
//            /**
//             * 把新的属性插入旧属性集中替换已有,或新增.
//             */
//            Map<String, Object> completeAttribues = oldManticoreStorageEntity.getJsonFields();
//            Map<String, Object> modifiedAttributes = serializeToMap(attribute, true);
//            completeAttribues.putAll(modifiedAttributes);
//            // 处理 fulltext
//            oldManticoreStorageEntity.setJsonFields(completeAttribues);
//            oldManticoreStorageEntity.setFullFields(convertJsonToFull(completeAttribues));
//            doBuildReplaceStorageEntity(oldManticoreStorageEntity, true);
//        } else {
...
//        //  jsonFields
//        manticoreStorageEntity.setJsonFields(serializeToMap(entityValue, true));
//        // fullTexts
//        manticoreStorageEntity.setFullFields(serializeSetFull(entityValue));
//    }
//
//    @Override
...
//        long startMs = System.currentTimeMillis();
//        try {
//            checkId(manticoreStorageEntity.getId());
//            try {
//                entityValueToStorage(manticoreStorageEntity, entityValue);
//            } catch (Exception e) {
//                throw new SQLException(
//                    String.format("serialize entityValue to jsonFields/fullTexts failed, message : %s", e.getMessage()), PARSE_COLUMNS_ERROR.name());
//            }
//
//            return doBuildReplaceStorageEntity(manticoreStorageEntity, replacement);
//        } finally {
...
//
//    @Override
//    public int batchSave(Collection<ManticoreStorageEntity> storageEntities, boolean replacement, boolean forceRetry) throws SQLException {
//
//        long startMs = System.currentTimeMillis();
//        //  database key -> table key -> List
//        Map<String, List<ManticoreStorageEntity>> shardingStorageEntities = new LinkedHashMap<>();
//        //  分类storageEntity
//        for (ManticoreStorageEntity manticoreStorageEntity : storageEntities) {
//            //  写入到shardingStorageEntities中
//            String shardKey = Long.toString(manticoreStorageEntity.getId());
...
//                .add(manticoreStorageEntity);
//        }
//
//        //  开始按照数据库/表进行分片插入
//        int executed = 0;
//        for (Map.Entry<String, List<ManticoreStorageEntity>> entities : shardingStorageEntities.entrySet()) {
//            Collection<List<ManticoreStorageEntity>> partitions = Lists.partition(entities.getValue(), maxBatchSize);
//            for (List<ManticoreStorageEntity> manticoreStorageEntityList : partitions) {
//                try {
//                    int batchExecuted = doBatchSave(manticoreStorageEntityList, replacement);
//                    if (batchExecuted != manticoreStorageEntityList.size()) {
//                        throw new SQLException(
...
//                    for (ManticoreStorageEntity se : manticoreStorageEntityList) {
//                        try {
...
//
//    //    // 更新原始数据.
//    private int doBatchSave(List<ManticoreStorageEntity> storageEntities, boolean replacement) throws SQLException {
//
...
//
//    //    // 更新原始数据.
//    private int doBuildReplaceStorageEntity(ManticoreStorageEntity manticoreStorageEntity, boolean replacement) throws SQLException {
//        String shardKey = Long.toString(manticoreStorageEntity.getId());
//        return (int) writeTransactionExecutor.execute(new ResourceTask() {
//
//            @Override
...
//                        resource, indexWriteIndexNameSelector.select(shardKey)).execute(manticoreStorageEntity);
//                } else {
