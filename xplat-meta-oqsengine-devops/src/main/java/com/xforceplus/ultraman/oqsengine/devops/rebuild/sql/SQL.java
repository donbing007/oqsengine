...
public class SQL {
    ...
    public static final String UPDATE_SQL = "update %s set updatetime = ?, finishsize = ?, status = ?, message = ?, startid = ? " +
            "where maintainid = ? and status not in (2, 3, 4)";

    public static final String ERROR_SQL = "update %s set updatetime = ?, finishsize = ?, status = ?, message = ?, startid = ? " +
            "where maintainid = ? and status != 2";

    public static final String INCREASED_FINISH = "update %s set updatetime = ?, finishsize = ?, startid = ? " +
            "where maintainid = ? and status != 2";
