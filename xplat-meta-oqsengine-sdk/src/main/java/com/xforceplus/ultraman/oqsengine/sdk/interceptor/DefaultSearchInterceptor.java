...
public class DefaultSearchInterceptor<R> implements MessageDispatcherInterceptor<QueryMessage<ConditionSearchCmd, R>> {
    ...
    private boolean isEmptyCondition(ConditionQueryRequest request){
        return request.getConditions() == null ||
        ( (request.getConditions().getFields() == null) || (request.getConditions().getFields().isEmpty())  &&
          (request.getConditions().getEntities() == null || request.getConditions().getEntities().isEmpty())
        ) ;
    }
