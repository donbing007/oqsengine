...
public class SendUtils {
     ...
     */
    public static void sendRequest(RequestWatcher requestWatch, EntityClassSyncRequest entityClassSyncRequest
                                                    , Function<String, Boolean> checkFunction, String param) {
        /**
         * 这里由于异步执行了OQS的更新，等待后可能出现新的流始化了，所以必须进行doubleCheck判断uid是否相同
         */
        ...
        sendRequest(requestWatch, entityClassSyncRequest);
    }


    public static void sendRequest(RequestWatcher requestWatch, EntityClassSyncRequest entityClassSyncRequest) {
        try {
            requestWatch.observer().onNext(entityClassSyncRequest);
        } catch (Exception e) {
